...
// Generic mojibake repair (Mac Roman + Latin-1 reversal) — see encoding-fix.js.
// Replaces the old hard-coded pattern list, which missed new names (e.g. Tyler O'Neill)
// and had a bug that produced "Jesúss". Handles any player name automatically.
const { fixText: fixCharacterEncoding } = require('./encoding-fix.js');
...
const cardsData = rawData.map((row, index) => {
    return {
        ...
        team: fixCharacterEncoding(String(row.team || row.Team || '').trim()),
        year: parseInt(row.year || row.Year || 0) || null,
